...
def Model(data_x,data_m, data_age, gain_parameters, times):
    seed = 25 + times
    random.seed(seed)
    np.random.seed(seed)
    ...
    MSE_loss = tf.reduce_mean((M * X - M * G_sample) ** 2) / tf.reduce_mean(M) + tf.reduce_mean((AGE - Age_sample) ** 2)
    ...
    for it in tqdm(range(iterations)):
        ...
        _, MSE_loss_curr = \
            sess.run([G_solver, MSE_loss],
                     feed_dict={X: X_mb, M: M_mb, AGE: Age_mb})
        # if it % 1000 == 0:
        #     print(MSE_loss_curr)
    ## Return imputed data
    M_mb = data_m
...
def main (args,yy, times,disease_id):
  ...
  ori_data_x, miss_data_x, data_m, data_age = data_loader(miss_rate,disease_id)
  
  # Impute missing data
  imputed_data_x = Model(miss_data_x, data_m, data_age, gain_parameters, times)
...
if __name__ == '__main__':
    ...
    for disease_id in range(len(disease_list)):
        for missing_rate in missing_rate_list:
            file_name = "DME_" + str(missing_rate) + "_" + str(disease_list[disease_id]) + ".csv"
