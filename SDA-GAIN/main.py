...
def main (args,yy,times,disease_id):
  ...
  ori_data_x, miss_data_x, data_m, data_age = data_loader(miss_rate, yy, times,disease_id)
  ...
  for dd in range(len(disease_id)):
    ...
    RMSE, MAE, MAPE = test_loss(ori_data_x[dd,:,:],imputed_data_x[dd,:,:],data_m[dd,:,:])
    res.append(RMSE)
    res.append(MAE)
    res.append(MAPE)
  return res
...
if __name__ == '__main__':
    ...
    for disease_id2 in range(len(disease_list)):
        for missing_rate in missing_rate_list:
            ...
            file_name = "SDA-GAIN_" + str(missing_rate*10) + "_"+disease_list[disease_id[0]]+".csv"
